...
def load_pickle_data():
    ...
    return (
        df_all_residues, df_distant_residues, sorted_paths, num_bootstrap_samples,
        numpath, pdb, last_residue, graphdist
    )
...
def test_create_bootstrap_sample(load_dataframe, load_pickle_data):
    ...
    assert not bootstrap_sample.equals(load_dataframe), "Bootstrap sample should not be identical to the original data"
...
def test_process_bootstrap_sample(load_dataframe, load_pickle_data, setup_directories):
    ...
    assert all(isinstance(path, list) for path in bootstrap_pathways), "Each pathway should be a list"
    assert os.path.exists('bootstrap/bootstrap_sample_1.txt'), "Output file should be created"
...
def test_bootstrap_analysis(load_pickle_data, setup_directories):
    ...
    assert isinstance(common_counts, np.ndarray), "common_counts should be a numpy array"
    assert len(common_counts) == num_bootstrap_samples, "Length of common_counts should match num_bootstrap_samples"
    assert isinstance(path_confidence_intervals, dict), "path_confidence_intervals should be a dictionary"
...
def test_bootstrap_write(load_pickle_data, setup_directories, tmp_path):
    ...
    analysis.common_counts, analysis.path_confidence_intervals = analysis.bootstrap_analysis()
    ...
    for path, (mean, lower, upper) in analysis.path_confidence_intervals.items():
        ...
        assert expected_line in lines, f"Expected line '{expected_line.strip()}' not found in file"
