...
import re
import json
from mdpath.src.notebook_vis import NotebookVisualization  # Adjust import according to your module
...
def mock_json_data():
    ...
    return [
    {'clusterid': 1, 'color': [1, 0, 0], 'coord1': [0, 0, 0], 'coord2': [1, 1, 1], 'pathway_index': 0, 'radius': 0.5},
    {'clusterid': 1, 'color': [0, 1, 0], 'coord1': [1, 1, 1], 'coord2': [2, 2, 2], 'pathway_index': 1, 'radius': 0.5},
    {'clusterid': 2, 'color': [0, 0, 1], 'coord1': [2, 2, 2], 'coord2': [3, 3, 3], 'pathway_index': 2, 'radius': 0.5}
]
...
def test_generate_cluster_ngl_script(mock_view, mock_json_data):
    ...
    with patch("builtins.open", mock_open(read_data=json.dumps(mock_json_data))):
        ...
        assert normalize_script(expected_script_1) in [normalize_script(call) for call in calls]
        assert normalize_script(expected_script_2) in [normalize_script(call) for call in calls]